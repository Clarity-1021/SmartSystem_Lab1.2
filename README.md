...
### 1.1 卷积网络 `src/CNN_network.py`

设置了两个隐藏层。

### 1.2 加载数据 `src/load_data.py`
...
### 1.4 小工具 `src/tools.py`

- 用给定的网络、data_loader和损失函数，计算loss和正确率。
- 画训练集和开发集的正确率对比图。
- 画训练集和开发集的loss对比图。

### 1.5 测试网络 `src/test.py`

可以重新加载网络来测试别的测试集，由于没有新的数据，我这里是测试的还是我的开发集。

### 1.6 对比实验 `src/compare`

所有做对比实验用的代码都放在这个目录下面。

## 2 设计实验改进网络并论证

通过控制变量法，每次只改变一个可能会对网络产生影响的指标。用同一个`train_loader`和`develop_loader`跑不同网络，确保不同网络训练时每一次的测试集和开发集给的数据都是一致的。通过不同网络的loss和正确率的对比来选择最优的再进行下一项指标的测试，一步步进行优化。
...
全局学习率大一点，每一个样本就调一次效果最好的原因，可能是调整的次数增加，对每一个数据的适应力就更好了，不具有普适性，只对于我的网络来看。
...
由于不同的特征，有不同的数量级，线性组合红的结果占比各不相同，数量级大的特征产生的影响更大。进行标准化（归一化），相当于平均了每个特征的影响，使得loss更容易受收敛。
...
分辨率更高的图片的输入层的特征点（像素点）更多一点，在我这个网络下效果会比较好，但也不一定具有普适性。
...
这个也没有什么普适性，只是通过实验来检查不同网络对于这个分类任务的适应能力。手写体识别并不需要很深层次的网络，特征较为单一，但又不是那么单一，一层的神经网络效果又显著的没有两三层的好，大概就是这个样子吧。

### 2.7 测试网络

<center>
    <img src = "./lab_result/2/loss.png"  width = "48%"/>
    <img src = "./lab_result/2/right rate.png"  width = "48%"/>
</center>
<center><strong>图 10 - 用最终的网络的训练结果</strong></center>

看起来还不错，收敛的速度还挺快的，在开发集上分类正确率的峰值也能达到**99.167%**。
